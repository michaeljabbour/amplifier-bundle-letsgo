...
description: >-
    ...
    browser task. Triggers: "open a website", "fill out a form", "click a
...
- Navigate and interact with websites (forms, buttons, links)
- Extract data or text from web pages
- Take screenshots or generate PDFs of pages
- Test web applications end-to-end
- Automate login flows and save auth state for reuse
- Record browser sessions as video
- Automate Mobile Safari via iOS Simulator
...
- Always snapshot before interacting -- never guess at refs
- Use `--session-name` for any multi-step workflow to survive browser restarts
- Prefer `snapshot -i` over full-page snapshots to reduce noise
- Use `wait --load networkidle` after navigation to ensure page stability
- For debugging, use `--headed` to watch the browser in real-time
- Delegate browser tasks to sub-agents (Amplifier delegate) to isolate context cost
